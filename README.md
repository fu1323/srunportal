...
v2.1.1 更新（2026.5月末）
对日志功能完善  日志固定写到~/logs下面
（可选）提供了一个lua脚本，mac用户可配合hammerspoon加载，配置开机自启。必须给予
hammerspoon位置权限，否则他拿不到ssid信息（lua脚本自己需要修改，修改sh脚本位置和ssid名称，先要
有个sh脚本 里面用java-jar启动本java程序就行）还把sh脚本扩展名改成command 手动双击使用
init.lua里面有如何屏蔽mac烦人的portal弹窗的指引，执行完命令需要重启机器